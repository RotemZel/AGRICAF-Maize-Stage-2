...
## discover possible impacts of monthly factors over global soybean price        #
#                                                                              #       
# -----------------------------------------------------------------------------#
# Main r script                                                                #
################################################################################
...
# !!! Important remarks !!!:
# a) Before running the R code, specify a main working directory ('wd_main') or refer to a project (soybean)
# b) run section 2

wd_general <- "~/price_forecasting"
# define a specified main working directory 
wd_main <- paste0("~/price_forecasting/",c,"_prices")
setwd(wd_main) # set directory

dir.create(paste0(c,"_data"))
wd_data <- paste0(wd_main,"/",c,"_data")

################################################################################
#---------------- 1. create folders and specify directories for region -------- 

# define a specified main working directory 
wd_region <- paste0(wd_main,"/",c,"_region")
...
dir.create("analyse_production", showWarnings = F)
...
## mid-processed data, not ready to use
dir.create("forecast_production", showWarnings = F)
...
# define a specified main working directory 
wd_country <- paste0(wd_main,"/",c,"_country")
...
dir.create(paste0(c,"_after_forecast"), showWarnings = F)
wd_open <- paste0(wd_main,"/",c,"_after_forecast")

### processed
setwd(wd_open)
dir.create("processed", showWarnings = F)
wd_proc <- paste0(wd_main,"/",c,"_after_forecast/processed")
...
## mid-processed data, not ready to use
dir.create(paste0(c,"_ts"), showWarnings = F)
wd_ts <- paste0(wd_main, "/",c,"_ts")
setwd(wd_ts)
